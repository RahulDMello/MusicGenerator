...
def get_one_hot(data):
    ...
    b = np.zeros([len(temp), 1, each_note_size], np.int)
    b[np.arange(len(temp)), 0, temp] = 1
    return b
'''
...
def nn_model(data, config):
    ...
    # unrolling works here only because we are predicting after each time step 
    # if the time steps are more then we need to tf.gather the output for last time step
    output = tf.reshape(output, [-1, config['hidden_state'][-1]]) 
...
#print(length(input_train[0]))
n_epochs = 5
batch_size = len(input_train)//200

config = {'num_layers': 1, 'hidden_state': [178], 'n_output': 89, 'n_examples': len(input_train), 'batch_size': batch_size, 'learning_rate': 0.01}
